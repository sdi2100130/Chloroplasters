...
GREEN_LIGHT_PIN  = 21   # IO21 -> Green LED (normal operation)
...
def wait_for_button_release():
    """Wait until the button is released so one press only counts once."""
    while is_button_pressed():
        time.sleep(0.05)

def all_outputs_off():
    """Put pumps, fan, and state LEDs into a safe OFF condition."""
    global is_watering
...
# MAIN PROGRAM
# =========================================================
def homeostasis_state():
    # Initialize all DC Pumps to OFF
    all_outputs_off()
    show_normal_state()
    print("NORMAL STATE: Homeostasis is active.")
...
def safe_state():
    """Safe state: outputs stay off, red LED stays on, code keeps running."""
    all_outputs_off()
    show_safe_state()
    print("SAFE STATE: System paused. Press the button again to resume.")
...
def main():
    ...
    current_state = homeostasis_state
    while True:
